      ...
      <main className="flex-1 overflow-y-auto px-8 py-8">
        <div className="mx-auto max-w-3xl">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="font-display text-2xl font-semibold text-ink dark:text-[#ececed]">
                Números do WhatsApp
              </h1>
            ...
            <span className="shrink-0 rounded-full bg-mist px-3 py-1.5 text-xs font-semibold text-ink/60 dark:bg-[#24252e] dark:text-[#ececed]/60">
          ...
          <div className="mt-6 space-y-3">
            ...
            {!isLoading && sessions?.length === 0 && (
              <div className="rounded-xl border border-dashed border-line px-6 py-10 text-center dark:border-[#34353f]">
                <p className="text-sm text-ink/50 dark:text-[#ececed]/50">Nenhum número conectado ainda.</p>
              </div>
            )}
            ...
            {sessions?.map((session) => (
              <div
                key={session.id}
                className="flex items-center justify-between gap-4 rounded-xl border border-line bg-paper px-4 py-3.5 dark:border-[#34353f] dark:bg-[#1a1b21]"
              >
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-ink dark:text-[#ececed]">{session.label}</p>
                  <p className="mt-0.5 font-mono text-xs text-ink/40 dark:text-[#ececed]/40">
                    {session.phoneNumber ?? 'Ainda não pareado'}
                  </p>
                </div>
                <div className="flex shrink-0 items-center gap-4">
                  ...
                  {session.status === 'CONNECTED' ? (
                    <button
                      ...
                      className={`text-xs font-medium text-ink/50 hover:text-stage-lost dark:text-[#ececed]/50 ${PRESS}`}
                    >
                      Desconectar
                    </button>
                  ) : (
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className={`text-xs font-medium text-ink/50 hover:text-stage-lost disabled:opacity-50 dark:text-[#ececed]/50 ${PRESS}`}
                      >
                        Remover
                      </button>
                    </div>
          ...
          {atLimit && (
            <p className="mt-2 text-xs text-ink/40 dark:text-[#ececed]/40">
              Limite de {MAX_SESSIONS} números atingido.
            </p>
          )}
        </div>
      </main>
