  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/60 px-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl bg-paper p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <p className="text-xs font-semibold uppercase tracking-wide text-ink/40">Ligação</p>
        ...
        <label className="mt-4 block text-sm font-medium text-ink/70">
          ...
          <input
            ...
            disabled={isFallback}
            className="mt-1 w-full rounded-lg border border-line px-3 py-2 text-sm text-ink outline-none focus:border-signal disabled:bg-mist disabled:text-ink/40"
          />
