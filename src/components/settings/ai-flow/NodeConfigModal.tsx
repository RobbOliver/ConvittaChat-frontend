...
export function NodeConfigModal({ node, outgoingEdgeLabels, onClose, onSave }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/60 px-4" onClick={onClose}>
      <div
        className="max-h-[85vh] w-full max-w-md overflow-y-auto rounded-2xl bg-paper p-6 shadow-xl dark:bg-[#1a1b21]"
        onClick={(e) => e.stopPropagation()}
      >
        <p className="text-xs font-semibold uppercase tracking-wide text-ink/40 dark:text-[#ececed]/40">{TYPE_LABEL[node.type]}</p>
        <h2 className="mt-1 font-display text-lg font-semibold text-ink dark:text-[#ececed]">Configurar passo</h2>
        ...
        <label className="mt-4 block text-sm font-medium text-ink/70 dark:text-[#ececed]/70">
          ...
          <input
            ...
            className="mt-1 w-full rounded-lg border border-line px-3 py-2 text-sm text-ink outline-none focus:border-signal dark:border-[#34353f] dark:bg-[#1a1b21] dark:text-[#ececed]"
          />
        </label>
        ...
        {node.type === 'AI_MESSAGE' && (
          <label className="mt-4 block text-sm font-medium text-ink/70 dark:text-[#ececed]/70">
            ...
            <textarea
              ...
              className="mt-1 w-full resize-none rounded-lg border border-line px-3 py-2 text-sm text-ink outline-none focus:border-signal dark:border-[#34353f] dark:bg-[#1a1b21] dark:text-[#ececed]"
            />
            <span
              className={`mt-1 block text-xs ${instructions.length > INSTRUCTIONS_SOFT_LIMIT ? 'text-signal' : 'text-ink/40 dark:text-[#ececed]/40'}`}
            >
            ...
            <p className="mt-4 text-sm font-medium text-ink/70 dark:text-[#ececed]/70">
              ...
              {(['NONE', 'HUMAN_NEEDED', 'ORDER_READY'] as const).map((option) => (
                <button
                  ...
                  className={`rounded-full border px-3 py-1.5 text-xs font-medium ${PRESS_SM} ${
                    notify === option
                      ? 'border-signal bg-signal/10 text-ink dark:bg-signal/20 dark:text-[#ececed]'
        ...
        {node.type === 'END' && (
          <>
            ...
            <p className="mt-4 text-sm font-medium text-ink/70 dark:text-[#ececed]/70">Quando o fluxo terminar aqui</p>
            <div className="mt-2 flex flex-wrap gap-2">
              ...
              <label className="mt-4 block text-sm font-medium text-ink/70 dark:text-[#ececed]/70">
                ...
                <div className="mt-1 flex gap-2">
                  <input
                    ...
                    className="w-full rounded-lg border border-line px-3 py-2 text-sm text-ink outline-none focus:border-signal dark:border-[#34353f] dark:bg-[#1a1b21] dark:text-[#ececed]"
                  />
                  <select
                    value={restartLockUnit}
                    onChange={(e) => setRestartLockUnit(e.target.value as 'MINUTES' | 'HOURS')}
                    className="rounded-lg border border-line px-2 py-2 text-sm text-ink outline-none focus:border-signal dark:border-[#34353f] dark:bg-[#1a1b21] dark:text-[#ececed]"
                  >
        ...
        {node.type === 'WAIT_REPLY' && (
          <div className="mt-4">
            <p className="text-sm font-medium text-ink/70 dark:text-[#ececed]/70">
            ...
            {waitAssignKind === 'FIELD' && (
              <select
                ...
                className="mt-2 w-full rounded-lg border border-line px-3 py-2 text-sm text-ink outline-none focus:border-signal dark:border-[#34353f] dark:bg-[#1a1b21] dark:text-[#ececed]"
              >
        ...
        {node.type === 'CONDITION' && (
          <div className="mt-4">
            ...
            {conditionMode === 'FIELD' && (
              <div className="mt-3 space-y-3">
                {rules.map((rule, index) => (
                  <div key={index} className="rounded-lg border border-line p-3 dark:border-[#34353f]">
                    <div className="flex items-center gap-2">
                      <select
                        value={rule.field}
                        onChange={(e) => updateRule(index, { field: e.target.value })}
                        className="min-w-0 flex-1 rounded-lg border border-line px-2 py-1.5 text-xs text-ink outline-none focus:border-signal dark:border-[#34353f] dark:bg-[#1a1b21] dark:text-[#ececed]"
                      >
                        {fieldOptions.map((f) => (
                      ...
                      <select
                        ...
                        className="rounded-lg border border-line px-2 py-1.5 text-xs text-ink outline-none focus:border-signal dark:border-[#34353f] dark:bg-[#1a1b21] dark:text-[#ececed]"
                      >
                      ...
                      <button
                        ...
                        title="Remover regra"
                        className="shrink-0 text-ink/40 hover:text-stage-lost dark:text-[#ececed]/40"
                      >
                        ×
                      </button>
                    </div>
                    ...
                    {(rule.operator === 'equals' || rule.operator === 'notEquals') && (
                      <input
                        ...
                        placeholder="Valor"
                        className="mt-2 w-full rounded-lg border border-line px-2 py-1.5 text-xs text-ink outline-none focus:border-signal dark:border-[#34353f] dark:bg-[#1a1b21] dark:text-[#ececed]"
                      />
                    )}
                ...
                <button
                  ...
                  className={`rounded-full border border-line px-3 py-1.5 text-xs font-medium text-ink/70 hover:bg-mist dark:border-[#34353f] dark:text-[#ececed]/70 dark:hover:bg-[#24252e] ${PRESS_SM}`}
                >
                  + regra
                </button>
              </div>
                  ...
                  <div key={index} className="rounded-lg border border-line p-3 dark:border-[#34353f]">
                    <div className="flex items-start gap-2">
                      <textarea
                        ...
                        className="min-w-0 flex-1 resize-none rounded-lg border border-line px-2 py-1.5 font-mono text-xs text-ink outline-none focus:border-signal dark:border-[#34353f] dark:bg-[#1a1b21] dark:text-[#ececed]"
                      />
                      <button
                        type="button"
            ...
            <p className="mt-3 text-xs text-ink/40 dark:text-[#ececed]/40">
        ...
        {node.type === 'TRIGGER' && (
          <p className="mt-4 text-sm text-ink/50 dark:text-[#ececed]/50">Sempre o ponto de partida — toda conversa nova começa por aqui.</p>
        )}
